...
Installation adds Orkhon Code to Applications and opens its welcome page automatically. Recommended text and source-file groups start selected. Formats are grouped by their current app. Use its group control to select or unselect a whole group, or search by extension or app. Deselect any exceptions, then choose “Apply defaults and start editing.” Each group shows its current or recorded previous app. Markdown, C++ source, and C++ headers are eligible. macOS shares C++ source defaults with .cp, which is shown explicitly in that group. “Keep all current defaults” starts editing without changing associations. Revisit the screen through Orkhon Code → File Defaults.
