...
with zipfile.ZipFile(root/'outputs/Orkhon Code Source.zip','w',zipfile.ZIP_DEFLATED,compresslevel=6) as archive:
 for item in ['.gitignore','.github','Makefile','CONTRIBUTING.md','SECURITY.md','Package.swift','README.md','LICENSE','Sources','Tests','Resources','Vendor','scripts','docs']:
  path=root/item
  for f in ([path] if path.is_file() else sorted(path.rglob('*'))):
   if not f.is_file() or any(p in {'.git','.build','__pycache__','node_modules'} for p in f.parts):continue
