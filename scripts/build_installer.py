...
subprocess.run(['pkgbuild','--root',str(payload),'--identifier','app.orkhon.editor','--version','1.4.0','--install-location','/','--component-plist',str(component),'--scripts',str(scripts),str(stage/'OrkhonComponent.pkg')],check=True)
resources=stage/'resources';resources.mkdir()
(resources/'welcome.html').write_text('''<html><head><meta charset="utf-8"></head><body style="font-family:-apple-system;font-size:13px;color:#293344"><h1 style="font-size:26px">Orkhon Code</h1><p>new document: here is a little room to think.</p><p>A small, native editor for text and code.</p><p>This installer places Orkhon Code in Applications. Quit any running copy before continuing.</p><p>On first launch, review the recommended source-file defaults and deselect any you want to keep. Nothing changes until you confirm. Browser, media, design, and ambiguous file formats keep their current apps.</p><p>For Apple silicon Macs running macOS 13 or later.</p></body></html>''')
...
<pkg-ref id="app.orkhon.editor" version="1.4.0" onConclusion="none">OrkhonComponent.pkg</pkg-ref>
