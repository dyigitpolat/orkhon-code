...
extension EditorWindowController {
    ...
    func openRemoteFile(_ path:String) {
        ...
        Task { [weak self,weak d] in
            ...
            d.loading=false
            switch result {
            case .success(let file):d.format=file;d.editor.send(2171,w:0,l:0);d.editor.text=file.text;d.editor.markSaved();d.language=LanguageRegistry.shared.language(for:URL(fileURLWithPath:path),text:String(file.text.prefix(200)));self.configureLanguage(d);self.updateStatus();self.rebuildTabs();self.revealSelectedTab()
    ...
    func saveRemoteDocument(_ d:DocumentTab)->Bool {
        ...
        do {
            ...
            let panel=NSPanel(contentRect:NSRect(x:0,y:0,width:360,height:120),styleMask:[.titled],backing:.buffered,defer:false);panel.title="Saving to \(connection.host)";panel.isReleasedWhenClosed=false
            ...
            var failure:Error?
            Task {
                let result=await Task.detached{Result{try connection.write(path,data:data,expected:format.originalData)}}.value
            ...
            d.format=try DocumentStorage.decode(data);d.editor.markSaved();clearRecovery(d);rebuildTabs();updateStatus();return true
