...
HTML, SVG, browser shortcuts, media, and ambiguous binary formats are excluded from default-app setup. Eligibility depends on the reviewed text format, **not its current application**. Ordinary `.log` files in Console, `.csv`/`.tsv` files in Numbers, text in TextEdit, and supported source files in any other app are selectable. The current app is shown so you can uncheck exceptions before confirming. Native Numbers spreadsheets and Console diagnostic archives remain excluded. `.ts` and `.mts` can mean video; use **Open With → Orkhon Code** or **Open File** for TypeScript files with those suffixes without replacing video defaults. `.tsx` and `.cts` are included. C++ source files share a macOS setting with `.cp`; the setup explicitly shows that alias.

**Open With is available independently of defaults.** The installed app declares the extensions from its syntax catalog, plus the reviewed text extensions, as an alternate editor. This includes HTML and SVG source even though their defaults are not offered for replacement. Choosing **Keep all current defaults** or unchecking a format does not remove Orkhon from Finder's **Open With** menu. Registration does not write a default-app preference or override an explicit user choice; if no preference exists, macOS chooses among the available apps. A shared suffix can also make Orkhon appear for a non-text file; only open its text variant in the editor.

You do not need Full Disk Access or Accessibility permission to change defaults. If macOS rejects a choice, setup explains the failure and leaves the controls editable. You can adjust the selection and retry; successful changes and your opt-outs are retained. macOS presents any required consent prompt itself.
